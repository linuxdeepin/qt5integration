...
bool Style::drawScrollBarSliderControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const QBrush &background = m_palette->brush(PaletteExtended::ScrollBar_HandleBrush, enabled, mouseOver, hasFocus, sunken, false, Qt::lightGray);
    const QBrush &border = m_palette->brush(PaletteExtended::ScrollBar_HandleBorderBrush, enabled, mouseOver, hasFocus, sunken, false, background);
